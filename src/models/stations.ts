...
export const checkStation = async (
  stationName: string, 
  argStationsTable: string = stationsTable, 
  ...
  return await db.oneOrNone( 
    ...
    ', [stationName, argStationsTable, argStationBranchTable, argBranchesTable]
  );
};

export const getStations = async (argStationsTable: string = stationsTable) => {
  const arr = await db.many(
    'SELECT (station_name) FROM $1:name', [argStationsTable]
  );
